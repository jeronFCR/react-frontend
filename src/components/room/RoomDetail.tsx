...
const RoomDetail: React.FC<DetailProps> = ({
  room,
  loading,
  selectDevice,
  handleRoomAction,
      ...
      {loading ? (
        ...
        <>
          ...
          <h3 className="mt-4 text-lg font-semibold">
            {t("room.detail.devices-section.title")}
          </h3>
          ...
          <h3 className="mt-4 text-lg font-semibold">
            {t("room.detail.book-section.title")}
          </h3>
          {room.booking ? (
            ...
            <p className="text-gray-500">
              {t("room.detail.book-section.no-booking")}
            </p>
          )}

          <Button
            dataTestId="room-detail-action-button"
            className="w-full"
            onClick={() => handleRoomAction(room.booking ? "RELEASE" : "BOOK")}
          >
            {room.booking
              ? t("room.detail.actions.release-button")
              : t("room.detail.actions.book-button")}
          </Button>
        </>
      ) : (
